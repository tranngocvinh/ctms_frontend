...
import { updateSupplier } from "../api/client";
    ...
    return (
        <div className="p-field p-col-12">
            <input
                ...
                style={{display: 'none'}}
            />
            <Button type="button" label={label || 'Chọn ảnh'} onClick={handleButtonClick}
...
const UpdateSupplierForm = ({ fetchContainers, container }) => {
    const initialValues = {
        name: container?.name || '',
        address: container?.address || '',
        detailService: container?.detailService || '',
        email: container?.email || '',
        website: container?.website || '',
        phoneNumber: container?.phoneNumber || '',
        image: container?.imageData || null,
    };
            ...
            <div className="p-col-12 p-md-8">
                <Formik
                    ...
                    validationSchema={Yup.object({
                        ...
                        detailService: Yup.string().min(10, 'Vui lòng nhập mô tả có độ dài lớn hơn 10 kí tự').required('Không được để trống'),
                        ...
                        phoneNumber: Yup.string().matches(/^[0-9]{10}$/, 'Vui lòng nhập đúng định dạng số điện thoại').required('Không được để trống'),
                        image: Yup.mixed().required('Vui lòng tải lên một hình ảnh'),
                    })}
                    onSubmit={(values, { setSubmitting }) => {
                        const { image, ...data } = values;
                        updateSupplier(container.supplierId, data, image)
                            .then(res => {
                                fetchContainers();
